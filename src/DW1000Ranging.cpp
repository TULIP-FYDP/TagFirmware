...
volatile uint8_t DW1000RangingClass::_currentDeviceIndex = 0; // For Round Robin
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			if(_type == ANCHOR) {
				...
				if(messageType == POLL_ACK) {
                    ...
                    // Serial.print(_currentDeviceIndex);
                    // Serial.print(" ");
                    // Serial.println(myDistantDevice->getShortAddress(), HEX);
                    ...
                    // XXX
                    // Serial.print("Range ACK Received ");
                    // Serial.print(_currentDeviceIndex);
                    // Serial.print(" ");
                    // Serial.println(myDistantDevice->getShortAddress(), HEX);
                    ...
                    // Serial.print((uint32_t)myDistantDevice->timePollSent.getTimestamp());
                    // Serial.print(" ");
                    // Serial.print((uint32_t)myDistantDevice->timePollReceived.getTimestamp());
                    // Serial.print(" ");
                    // Serial.print((uint32_t)myDistantDevice->timePollAckSent.getTimestamp());
                    // Serial.print(" ");
                    // Serial.print((uint32_t)myDistantDevice->timePollAckReceived.getTimestamp());
                    // Serial.print(" ");
                    // Serial.print((uint32_t)myDistantDevice->timeRangeSent.getTimestamp());
                    // Serial.print(" ");
                    // Serial.print((uint32_t)myDistantDevice->timeRangeReceived.getTimestamp());
                    // Serial.println(" ");

                     _currentDeviceIndex++;
                    if (_currentDeviceIndex >= _networkDevicesNumber) {
...
void DW1000RangingClass::transmitPoll(DW1000Device* myDistantDevice) {
    // XXX
    // Serial.println("");
    // Serial.print("trans: POLL ");
    // Serial.println(_currentDeviceIndex);

	transmitInit();

	if(myDistantDevice == NULL) {
        uint8_t i = _currentDeviceIndex;
		...
		_globalMac.generateShortMACFrame(data, _currentShortAddress, _networkDevices[i].getByteShortAddress());
		...
		// for(uint8_t i = 0; i < _networkDevicesNumber; i++) {
			...
			_networkDevices[i].setReplyTime((+1)*DEFAULT_REPLY_DELAY_TIME);
			//we write the short address of our device:
			memcpy(data+SHORT_MAC_LEN+2, _networkDevices[i].getByteShortAddress(), 2);
		...
		copyShortAddress(_lastSentToShortAddress, _networkDevices[i].getByteShortAddress());
    ...
    // DW1000Device pollTarget = _networkDevices[_currentDeviceIndex];
    // Serial.print(pollTarget.getShortAddress(), HEX);

	// // 	_timerDelay = DEFAULT_TIMER_DELAY;
	// _timerDelay = DEFAULT_TIMER_DELAY+(uint16_t)(1*3*DEFAULT_REPLY_DELAY_TIME/1000);

    // // _globalMac.generateShortMACFrame(data, _currentShortAddress, pollTarget.getByteShortAddress());
	// byte shortBroadcast[2] = {0xFF, 0xFF};
	// _globalMac.generateShortMACFrame(data, _currentShortAddress, shortBroadcast);

    // data[SHORT_MAC_LEN]   = POLL;
    // data[SHORT_MAC_LEN+1] = 1;
    // uint16_t replyTime = pollTarget.getReplyTime();
    // memcpy(data+SHORT_MAC_LEN+2, &replyTime, sizeof(uint16_t)); // todo is code correct?

    // copyShortAddress(_lastSentToShortAddress, pollTarget.getByteShortAddress());
...
void DW1000RangingClass::transmitRange(DW1000Device* myDistantDevice) {
	...
	if(myDistantDevice == NULL) {
		...
		// for(uint8_t i = 0; i < _networkDevicesNumber; i++) {
			...
			//we get the device which correspond to the message which was sent (need to be filtered by MAC address)
			_networkDevices[i].timeRangeSent = timeRangeSent;
			_networkDevices[i].timePollSent.getTimestamp(data+SHORT_MAC_LEN+4);
			_networkDevices[i].timePollAckReceived.getTimestamp(data+SHORT_MAC_LEN+9);
			_networkDevices[i].timeRangeSent.getTimestamp(data+SHORT_MAC_LEN+14);
